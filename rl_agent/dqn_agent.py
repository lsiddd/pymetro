...
class CuriosityModule(nn.Module):
    def __init__(self, state_size, action_size, feature_dim=256):
    ...
    def calculate_curiosity(self, state, action, next_state):
...
class PrioritizedReplayBuffer:
    ...
    """
    def __init__(self, action_size, buffer_size, batch_size, seed, device, alpha=0.6, beta=0.4, beta_increment=0.001):
        ...
        self.epsilon = 1e-6  # Small value to prevent zero priority
    ...
    def sample(self):
        ...
        weights = (priorities / self.tree.total() * self.tree.n_entries) ** (-self.beta)
...
class DQNAgent:
    def __init__(self, state_size, action_size, config: RLAgentConfig, seed=0):
        ...
        self.memory = PrioritizedReplayBuffer(action_size, config.BUFFER_SIZE, config.BATCH_SIZE, seed, self.device)
        
        # Curiosity Module
        self.curiosity_module = CuriosityModule(state_size, action_size).to(self.device)
