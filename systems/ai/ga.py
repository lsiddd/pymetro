...
class GeneticAlgorithmTask(threading.Thread):
    ...
    def run(self):
        ...
        for g in range(1, G_max + 1):
            ...
            # Evaluate
            fitnesses = [calculate_fitness(p, self.game_state, seed_atual, H) for p in population]
            
            gen_best_idx = fitnesses.index(max(fitnesses))
            gen_best_fit = fitnesses[gen_best_idx]
            
            print(f"[AG] Geração {g}/{G_max} - Melhor deste ciclo: {gen_best_fit:.2f}")
